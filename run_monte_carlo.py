...
#
# Usage:
#   python run_monte_carlo.py [--runs N] [--seed S] [--output DIR] [--config PATH]
#
# The script generates a unique YAML configuration for each simulation run
# with realistic parameter variations, runs the simulation, and collects results.
...
BASE_YAML_TEMPLATE = """# ======================================================================
# Simulation configuration for Anopheles stephensi – Monte Carlo Run {run_id}
# Seed: {seed}
# ======================================================================
...
# ======================================================================
# PARAMETER RANGES - Matches your SpeciesParameters class
# ======================================================================
...
PARAMETER_RANGES = {
    ...
    'resting_energy_threshold': {
        ...
        'max': 0.95,
        'base': 0.9,
        'description': 'Energy threshold for resting',
        'category': 'behavior'
    },
...
def generate_parameter_set(seed: int, param_ranges: Dict = PARAMETER_RANGES) -> Dict:
    ...
    # Temperature thresholds in Kelvin should be consistent
...
def generate_yaml_config(seed: int, params: Dict, output_dir: str, run_id: int, 
                         template: str = BASE_YAML_TEMPLATE) -> str:
    ...
    try:
        ...
        print(f"Warning: Missing parameter in template: {e}")
        # Fill missing with defaults
        template_with_defaults = template
        for key in params:
            if key not in template_with_defaults:
                template_with_defaults = template_with_defaults.replace(
                    f"{{{key}}}", str(params[key])
                )
        return template_with_defaults.format(
            run_id=run_id,
            seed=seed,
            output_dir=output_dir_str,
            **params
...
def run_simulation(seed: int, output_dir: str, jar_path: Optional[str] = None, 
    ...
    # Build command
    # The JAR expects: seed outputDir configPath
    cmd = [
        "java",
        "-jar", jar_path,
        str(seed),
...
def run_monte_carlo(n_runs: int, base_seed: Optional[int] = None, 
                   ...
                   config_template: Optional[str] = None) -> Tuple[List, List]:
    """
    ...
    Args:
        ...
        config_template: Custom YAML template
    ...
    for i in range(n_runs):
        ...
        if vary_params:
            ...
            if verbose:
                ...
                # Show key parameters by category
                categories = {}
