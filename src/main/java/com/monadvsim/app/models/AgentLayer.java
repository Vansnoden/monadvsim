...
public class AgentLayer extends Layer {
    ...
    @JsonIgnore
    private byte[][] terrainCache;
    @JsonIgnore
    private Envelope cacheBounds;
    @JsonIgnore
    private double gridResX, gridResY;
    @JsonIgnore
    private GeometryFactory gf = new GeometryFactory();
    @JsonIgnore
    private Map<Byte, String> terrainLookup = new HashMap<>();
    ...
    private void initSpatialIndexing(STRtree index, int res, Consumer<Integer> progress) {
        for (int i = 0; i < res; i++) {
            ...
            if (i % 25 == 0 && progress != null) {
                progress.accept((int) ((i / (double) res) * 100));
            }
        }
    }
    ...
    public void bakeTerrainCache(Project project, int res, Consumer<Integer> progress) {
        ...
        if (cacheBounds == null || cacheBounds.isNull()) {
            return;
        }
    ...
    public Envelope getSimulationExtent(Project project) {
        ...
        if (env.isNull()) {
            return project.getCrsCode().contains("3857")
