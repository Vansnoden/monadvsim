...
public class AgentLayer extends Layer{
  ...
  private AgentRule rule = new AgentRule(); 
  ...
  public boolean validateMovement(double x, double y) {
    ...
    return rule.isAllowed(terrainType);
  }
  ...
  /**
  * Rapid O(1) lookup into the baked binary grid.
  */
  public String fastDetectTerrain(double x, double y) {
    ...
    if (col >= 0 && col < terrainCache.length && row >= 0 && row < terrainCache[0].length) {
      ...
      if (val == 1) return "land";
      if (val == 2) return "water";
    }
    return "empty"; 
  }
  ...
  */
  public void bakeTerrainCache(Project project, int resolution, Consumer<Integer> progressListener) {
          ...
          try {
            ...
            try (FeatureIterator<?> it = source.getFeatures().features()) {
              while (it.hasNext()) {
                ...
                if (g != null) {
                  // Insert geometry with layer context for terrain identification
    ...
    for (int i = 0; i < resolution; i++) {
      ...
      IntStream.range(0, resolution).parallel().forEach(j -> {
        ...
        byte cellValue = 0; // Default empty
        for (Object o : results) {
          Object[] data = (Object[]) o;
          Geometry geom = (Geometry) data[0];
          String layerName = (String) data[1].toString().toLowerCase();
          if (geom.contains(p)) {
            cellValue = layerName.contains("water") ? (byte)2 : (byte)1;
            break;
          }
        }
  ...
  public AgentRule getRule() { return rule; }
  public void setRule(AgentRule rule) { this.rule = rule; }
