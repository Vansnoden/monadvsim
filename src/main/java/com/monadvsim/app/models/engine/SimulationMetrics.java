...
public class SimulationMetrics {
    ...
    public void recordAgentProcessed() { agentsProcessed.incrementAndGet(); }
    public void recordRuleEvaluated() { rulesEvaluated.incrementAndGet(); }
    public void recordSpatialQuery() { spatialQueries.incrementAndGet(); }
    public void recordBirth() { births.incrementAndGet(); }
    public void recordDeath() { deaths.incrementAndGet(); }
    
    public void printReport() {
        double elapsedSeconds = (System.currentTimeMillis() - startTime) / 1000.0;
        SimulationLogger.info("\n=== SIMULATION METRICS ===");
        SimulationLogger.info("Elapsed Time: %.2f seconds%n", elapsedSeconds);
        SimulationLogger.info("Agents Processed: %,d (%.0f/sec)%n", agentsProcessed.get(), agentsProcessed.get() / elapsedSeconds);
        SimulationLogger.info("Rules Evaluated: %,d (%.0f/sec)%n", rulesEvaluated.get(), rulesEvaluated.get() / elapsedSeconds);
