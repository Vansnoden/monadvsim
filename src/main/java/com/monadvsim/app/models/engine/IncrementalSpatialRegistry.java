 ...
 */
public class IncrementalSpatialRegistry {
    ...
    public IncrementalSpatialRegistry(Rectangle2D worldBounds, double cellSize) {
    ...
    private void initializeGrid() {
        for (int x = 0; x < gridWidth; x++) {
            for (int y = 0; y < gridHeight; y++) {
                spatialGrid.put(new GridCell(x, y), Collections.synchronizedList(new ArrayList<>()));
            }
        }
    }
