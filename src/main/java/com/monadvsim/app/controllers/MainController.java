...
public class MainController {
    ...
    private void showTreeContextMenu(MouseEvent e, Layer layer) {
        ...
        inspect.addActionListener(a -> openDataTable(layer));
        m.add(inspect);
        m.addSeparator();
    ...
    private void handleLayerProperties(Layer l) {
        DialogLayerProperties d = new DialogLayerProperties(view, l);
        d.setVisible(true);
        if (d.isConfirmed()) {
    ...
    private void initMapTools() {
        ...
        view.getBtnFullview().addActionListener(e -> {
            view.getSimulationCanvas()
                    .setActiveTool(null);
            view.getSimulationCanvas().zoomToData();
        });
    }
    ...
    private void updateRecentMenu() {
        ...
        recentService.getRecentProjects().forEach(p -> {
            ...
            i.addActionListener(e -> handleOpenProject(new File(p)));
            m.add(i);
        });
    }
    ...
    private void showProjectContextMenu(MouseEvent e) {
        JPopupMenu m = new JPopupMenu();
        JMenuItem p = new JMenuItem("Properties");
        p.addActionListener(a -> handleProjectProperties());
        m.add(p);
        m.show(e.getComponent(), e.getX(), e.getY());
    }
    ...
    private void handleProjectProperties() {
        ...
        d.getBtnApply().addActionListener(e -> {
            project.setName(d.getProjectName());
            project.setCrsCode(d.getSelectedCrs());
            view.getSimulationCanvas().updateViewportCRS(project.getCrsCode());
            refreshUI();
            d.dispose();
        });
